...
<div style="width:80%; margin-left:10%">
  ...
  <form method="post" id="backForm" style="float:right">
    ...
    <div class=flex1 id="nuevo">
      <div class="a38">
        ...
        *Nombre del evento:
        <br><input type=text name="evento" id="evento" maxlength="35">
        <br><br>
        Cuándo :
        <br><input type=text name="cuando" id="cuando" maxlength="35">
        <br><br>
        Dónde :
        <br><input type=text name="donde" id="donde" maxlength="35">
      </div>
  ...
  <input class=boton1 type=button value="+ Agregar evento" onmouseup="addNew()" style="padding:0.6em 2.4em">
  </form>
</div>
...
<div id="subirbox" style="display:none; position:fixed; top:20px; width:500px; height:750px; margin-left:3%; z-index:1400; background-color:rgba(255,255,255,0.95); border-radius: 5px;box-shadow: 0px 0px 15px 0px rgba(0,0,0,0.5);">
  ...
  <form name="dzone" id="dzone" action="upload_foto_prod.php" class="dropzone">
    ...
    <input type=hidden name="upto" value="im/eventos">
  </form>
</div>
...
require_once "admin-eventos-lista.php";
...
function addNew(){
  ...
  if (!evento || evento.length < 3){  err += 'Favor de ingresar nombre del evento <br>';  }
  ...
  if (err.length > 1){
    ...
    var datos = $('#formEventos').serialize();
    $('#modal001').fadeIn();

    $.ajax({
        url: 'admin-eventos-insert.php',
